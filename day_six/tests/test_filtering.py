def should_apply_status_and_search_filters_together(client):
    client.post("/tasks", json={"title": "Buy milk", "status": "todo"})
    client.post("/tasks", json={"title": "Buy eggs", "status": "done"})
    client.post("/tasks", json={"title": "Call doctor", "status": "todo"})

    response = client.get("/tasks?status=todo&search=Buy")
