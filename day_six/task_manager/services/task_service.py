...
def _resolve_page_size(query: TaskListQuery) -> int:
    """Return the requested page size, falling back to the default."""
    return query.page_size or DEFAULT_PAGE_SIZE


def _build_page_response(
    tasks: List[Task], total: int, page: int, page_size: int
) -> Dict[str, Any]:
    """Build the standard offset-limit pagination envelope."""
    return {
...
def _build_cursor_response(
    page_tasks: List[Task], has_more: bool
) -> Dict[str, Any]:
    """Build the cursor-based pagination envelope."""
    return {
...
def _apply_sort(q: Query, query: TaskListQuery) -> Query:
    ...
    if query.sort not in SORTABLE_FIELDS:
        raise InvalidSortFieldError(
            f"'{query.sort}' is not a sortable field. Choose from: {sorted(SORTABLE_FIELDS)}"
        )
...
def list_tasks(query: TaskListQuery) -> Dict[str, Any]:
    ...
    page_size = _resolve_page_size(query)
    q = _apply_filters(Task.query, query)
